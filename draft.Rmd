...
For $n_{11}=7$ we have smaller probability (`r probs[8]`) than  $n_{11}=15$ - `r probs[16]`. 
However Information Gain from  $n_{11}=15$ is much higher
...
# indices2 <- 1:which.min(cumsum(z.df$unsort_prob[order(z.df$unsort_criterion, decreasing = T)])<0.05)

library(ggplot2)
library(latex2exp)
ggplot(z.df,aes(x=x,y=unsort_prob))+
  geom_density(aes(x=x,y=unsort_prob), color="black", stat = "identity") +
  geom_ribbon(data=z.df[1:9,], aes(ymin=0, ymax=unsort_prob, fill = "Common rejection", alpha=0.2)) +
  geom_ribbon(data=z.df[15:16,], aes(ymin=0, ymax=unsort_prob, fill = "Fisher's exact test", alpha=0.2)) +
  geom_ribbon(data=z.df[9:11,], aes(ymin=0, ymax=unsort_prob, fill = "QuiPT", alpha=0.2)) +
  scale_fill_discrete("Rejection region") + guides(alpha=FALSE) +
  xlab(TeX("n_{11}")) + ylab("Probability") +
  theme(legend.position="bottom") +
  ggtitle("Rejection regions for independence hypothesis at the level 0.05\nfor Fisher's exact test and QuiPT")
