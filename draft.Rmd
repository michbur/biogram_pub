...
test [cite], which verifies if a presence or an absence of a specific k-mer is
related to the status of the sequence. We propose new framework called QuiPT, an exact
version of permutation test, which is more accurate, faster and as customizable,
in terms of statistcs we measure, as permutation test itself.

### Permutation test

In this section let us 
consider target $y=(y_1, ..., y_n)$ 
and matrix of features $X\in M_{n\times p}$. More specifically, as we focus on filtering,
...
Let us consider the contingency table we get for $y$ and $x$:
...
random permutations. In fact, suppose we want to perform test with $\alpha=10^{-8}$, which is
...
When we test millions of features, there are just few distributions
that we need to compute and we take advantage of this. Therefore our complexity of our algorithm
